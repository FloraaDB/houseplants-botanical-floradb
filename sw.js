 ...
 * cross-origin endpoints (Web3Forms, Kaggle, iNaturalist S3, analytics).
 *
...
const CACHE_NAME = 'floradb-public-cache-v2026.07.2';
const CORE_ASSETS = [
  '/',
  '/favicon.svg',
  '/og-image.png',
...
self.addEventListener('activate', event => {
  event.waitUntil(
    caches.keys().then(keys =>
      Promise.all(keys.filter(k => k !== CACHE_NAME && k.startsWith('floradb-public-cache-')).map(k => caches.delete(k)))
